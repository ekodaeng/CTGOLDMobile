...
import { ArrowLeft, TrendingDown, AlertCircle, Calendar } from 'lucide-react';
import Card from '../components/Card';

export default function MemberBuyback() {
  const { member, isAuthenticated, isActive, isLoading } = useAuth();
  const [buybackRecords, setBuybackRecords] = useState<any[]>([]);
    ...
    <div className="min-h-screen bg-gray-950 px-4 py-8 pb-24">
      <div className="max-w-2xl mx-auto space-y-6 animate-fade-in">
        ...
        <div className="flex items-center space-x-3">
          <div className="bg-green-500/15 p-3 rounded-2xl border border-green-500/25">
            <TrendingDown className="text-green-400" size={28} />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-white">Laporan Buyback</h1>
        ...
        <Card className="bg-gradient-to-br from-green-500/10 to-gray-900/90 border-green-500/30">
          <div className="space-y-3">
            <h3 className="text-green-400 font-bold">Tentang Buyback CTGOLD</h3>
            <p className="text-gray-300 text-sm leading-relaxed">
              Buyback adalah proses pembelian kembali token CTGOLD dari pasar sebagai bagian dari kebijakan pengelolaan token.
              Aktivitas buyback dilakukan tanpa jadwal tetap dan bertujuan untuk transparansi komunitas.
            </p>
          </div>
        </Card>

        {buybackRecords.length === 0 ? (
          <Card className="bg-gradient-to-br from-gray-800/90 to-gray-900/90">
            <div className="text-center py-8 space-y-3">
              ...
              <p className="text-gray-400 text-sm">Belum ada record buyback saat ini</p>
              <p className="text-gray-500 text-xs">
                Record akan muncul di sini setelah ada aktivitas buyback
              </p>
            </div>
          </Card>
        ) : (
              ...
              <Card key={index} className="bg-gradient-to-br from-gray-800/90 to-gray-900/90">
                <div className="space-y-3">
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-green-400 text-sm font-semibold">Buyback</span>
                  </div>
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-white font-bold">${record.price}</p>
                    </div>
                  </div>
                  {record.txHash && (
                    <div>
        ...
        <Card className="bg-gray-800/50 border-gray-700/40">
          <div className="space-y-2 text-xs text-gray-400 leading-relaxed">
            ...
            <ul className="space-y-1">
              <li>• Buyback adalah aktivitas internal tim tanpa jadwal tetap</li>
              <li>• Tidak ada jaminan keuntungan atau buyback berkelanjutan</li>

...
function getAdminResetPasswordEmailTemplate(resetPasswordUrl: string, year: number): string {
  return `<!DOCTYPE html>
  ...
  <style type="text/css">
    ...
    * { box-sizing: border-box; }
    body {
      ...
      background-color: #070A0F; color: #E5E7EB;
    }
        ...
        <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="max-width: 520px; background-color: #0B0F1A; border-radius: 18px; border: 1px solid rgba(245, 197, 66, 0.18); box-shadow: 0 10px 40px rgba(0, 0, 0, 0.6);">
          ...
          <tr>
            <td style="padding: 24px 40px;" class="mobile-padding">
              <p style="margin: 0 0 16px 0; font-size: 15px; line-height: 1.6; color: #D1D5DB;">Halo Admin CTGOLD,</p>
              <p style="margin: 0 0 16px 0; font-size: 15px; line-height: 1.6; color: #D1D5DB;">Kami menerima permintaan untuk reset password akun admin CTGOLD.</p>
              <p style="margin: 0 0 24px 0; font-size: 15px; line-height: 1.6; color: #D1D5DB;">Jika permintaan ini benar berasal dari Anda, silakan klik tombol di bawah untuk membuat password baru.</p>
            </td>
          </tr>
          <tr>
            <td style="padding: 0 40px 32px 40px;" class="mobile-padding">
              <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%">
                <tr>
                  <td align="center">
                    <a href="${resetPasswordUrl}" target="_blank" class="button" style="display: inline-block; padding: 16px 48px; background: linear-gradient(135deg, #F5C542 0%, #D6B25E 100%); color: #0B0F1A; font-size: 16px; font-weight: 700; text-decoration: none; border-radius: 12px; box-shadow: 0 4px 16px rgba(245, 197, 66, 0.4); text-align: center; min-width: 200px;">Reset Password</a>
                  </td>
                </tr>
              </table>
            </td>
          ...
          <tr>
            <td style="padding: 0 40px 32px 40px;" class="mobile-padding">
              <p style="margin: 0 0 12px 0; font-size: 13px; line-height: 1.5; color: #9CA3AF; text-align: center;">Atau salin dan buka link berikut di browser Anda:</p>
              <p style="margin: 0; font-size: 12px; line-height: 1.5; word-break: break-all; text-align: center;">
                <a href="${resetPasswordUrl}" target="_blank" style="color: #F5C542; text-decoration: underline;">${resetPasswordUrl}</a>
              </p>
            </td>
          </tr>
          <tr>
              ...
              <table role="presentation" cellpadding="0" cellspacing="0" border="0" width="100%" style="background-color: rgba(59, 130, 246, 0.1); border: 1px solid rgba(59, 130, 246, 0.3); border-radius: 12px;">
                <tr>
                  <td style="padding: 20px;">
                    <p style="margin: 0 0 12px 0; font-size: 13px; font-weight: 600; color: #93C5FD; text-transform: uppercase; letter-spacing: 0.5px;">⚠️ Catatan Keamanan</p>
          ...
          <tr>
            <td style="padding: 24px 40px 40px 40px; text-align: center; border-top: 1px solid rgba(255, 255, 255, 0.05);" class="mobile-padding">
              <p style="margin: 0 0 8px 0; font-size: 12px; line-height: 1.5; color: #6B7280;">Email ini dikirim otomatis oleh sistem CTGOLD.<br>Mohon tidak membalas email ini.</p>
              <p style="margin: 0; font-size: 11px; line-height: 1.5; color: #4B5563;">© ${year} CTGOLD. All rights reserved.</p>
            </td>
          </tr>
        </table>
      </td>
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (!admin || !admin.is_active) {
      console.log(`Admin reset requested for invalid/inactive email: ${emailLower}`);
      return new Response(
        JSON.stringify({
          success: true,
          message: 'Jika email terdaftar sebagai admin, link reset password telah dikirim.',
        }),
        {
          status: 200,
          headers: { ...corsHeaders, 'Content-Type': 'application/json' },
    ...
    const origin = req.headers.get('origin') || req.headers.get('referer')?.split('/').slice(0, 3).join('/') || 'https://ctgold.io';
    const resetUrl = `${origin}/admin/reset-password?token=${resetToken}`;

    const emailHtml = getAdminResetPasswordEmailTemplate(resetUrl, new Date().getFullYear());

    const fromEmail = Deno.env.get('RESEND_FROM_EMAIL') || 'CTGOLD Security <no-reply@resend.dev>';

    const emailResult = await sendEmailWithResend(
      resendApiKey,
      fromEmail,
      emailLower,
      '🔐 Reset Password Admin CTGOLD',
      emailHtml
    );
    ...
    console.log('ADMIN RESET PASSWORD REQUEST (RESEND)');
    console.log('='.repeat(80));
